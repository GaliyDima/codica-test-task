...
export const getSavedCities:any = () => {
...
export const getWeather:any = (cityName: string, country: string) => {
...
export const getWeatherDetails:any = (cityName: string | undefined, lat: string, lon: string) => {
    return async (dispatch: any, getState: any) => {
        if (!lat || !lon) {
            getSavedCities();
            const state = getState();
...
export const updateWeather:any = (e: any, lat: string, lon: string) => {
    return async (dispatch: any) => {
        e.stopPropagation();
        try {
            const result = await axios.get(`https://api.openweathermap.org/data/2.5/weather?units=metric&lat=${ lat }&lon=${ lon }&appid=${ apiKey }`);
...
export const goTo:any = (e: any, name: string, navigate: any) => {
    return () => {
        navigate(`/details/${ name }`);
    }
}

export const getHourlyWeather:any = () => {
    return (dispatch: any) => {
